<?php get_header();?>
	<!-- Column 1 / Content -->
	<div class="grid_12">
		<h4 class="page_title">A full width page</h4>
		<div class="hr dotted clearfix">&nbsp;</div>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla vel diam id mauris accumsan egestas. Sed sed lorem. Integer id mi vel sapien fermentum vehicula. Pellentesque vitae lacus a sem posuere fringilla. Vestibulum dolor. Phasellus cursus augue ac purus. Curabitur faucibus. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas.</p>
		<!-- Floated Images within a paragraph -->
		<!-- Left -->
		<p><a href=""><img class="img_left" title="sampleimage" src="<?php bloginfo('template_url'); ?>/images/300x150.gif" alt="" /></a>Donec leo. Aliquam risus elit, luctus vel, interdum vitae, malesuada eget, elit. Nulla vitae ipsum. Donec ligula ante, bibendum sit amet, elementum quis, viverra eu, ante. Fusce tincidunt. Mauris pellentesque, arcu eget feugiat accumsan, ipsum mi molestie orci, ut pulvinar sapien lorem nec dui.</p>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. <b>Mauris vel porta erat.</b> Quisque sit amet risus at odio pellentesque sollicitudin. Proin suscipit molestie facilisis. Aenean vel massa magna. Proin nec lacinia augue. Mauris venenatis libero nec odio viverra consequat. In hac habitasse platea dictumst.</p>
		<!--Right -->
		<p><a href=""><img class="img_right" title="sampleimage" src="<?php bloginfo('template_url'); ?>/images/300x150.gif" alt="" /></a>Donec leo. Aliquam risus elit, luctus vel, interdum vitae, malesuada eget, elit. Nulla vitae ipsum. Donec ligula ante, bibendum sit amet, elementum quis, viverra eu, ante. Fusce tincidunt. Mauris pellentesque, arcu eget feugiat accumsan, ipsum mi molestie orci, ut pulvinar sapien lorem nec dui.</p>
		<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. <b>Mauris vel porta erat.</b> Quisque sit amet risus at odio pellentesque sollicitudin. Proin suscipit molestie facilisis. Aenean vel massa magna. Proin nec lacinia augue. Mauris venenatis libero nec odio viverra consequat. In hac habitasse platea dictumst.</p>
		<!-- Header's -->
		<h1>H1 Heading</h1>
		<h2>H2 Heading</h2>
		<h3>H3 Heading</h3>
		<h4>H4 Heading</h4>
		<h5>H5 Heading</h5>
		<h6>H6 Heading</h6>
		<!-- General html format's -->
		<p>This paragraph shows how all text encapsulated only within <strong>&lt;p&gt;&lt;/p&gt;</strong> tags will appear. Donec leo. Aliquam risus elit, luctus vel, interdum vitae, malesuada eget, elit. Nulla vitae ipsum. Donec ligula ante, bibendum sit amet, elementum quis, viverra eu, ante. Fusce tincidunt. Mauris pellentesque, arcu eget feugiat accumsan, ipsum mi molestie orci, ut pulvinar sapien lorem nec dui.</p>
		<p>More text examples are as follows:</p>
		<p><em>This is emphasised text</em></p>
		<p><strong>This is strong text</strong></p>
		<p><acronym title="This is an acronym - &lt;acronym&gt;&lt;/acronym&gt;">This is an acronym</acronym></p>
		<p><del>This is deleted text</del></p>
		<p><a href="#">This is a link</a></p>
		<blockquote>
			<p>This is a quote. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Duis tempor. Nullam tortor. Nulla vel dui. Curabitur et metus. This is a quote. Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Duis tempor. Nullam tortor. Nulla vel dui. Curabitur et metus.</p>
		</blockquote>
		<!-- List's -->
		<!-- Unordered -->
		<ul>
			<li>List item example</li>
			<li>List item example
				<ul>
					<li>Sub list item example</li>
					<li>Sub list item example</li>
				</ul>
			</li>
			<li>List item example</li>
		</ul>
		<!-- Ordered -->
		<ol>
			<li>List item example</li>
			<li>List item example
				<ol>
					<li>Sub list item example</li>
					<li>Sub list item example</li>
				</ol>
			</li>
			<li>List item example</li>
		</ol>
	</div>
	<div class="hr grid_12 clearfix">&nbsp;</div>
	<?php get_footer();?>
